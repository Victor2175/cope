...
def cross_validation_procedure(x,y,vars,\
    ...
    for idx_lambda, lambda_ in enumerate(lambda_range):
        ...
        if method == 'robust':
            ...
            w_tmp, rmse[(lambda_,1.0)], weights[(lambda_,1.0)], training_loss[(lambda_,1.0)] = \
                                        leave_one_out_procedure(x,y,vars,\
                                        lon_size,lat_size, notnan_idx,nan_idx,time_period,\
                                        'ridge', rank, lambda_, mu_range[0],\
                                        lr,nb_gradient_iterations,dtype=dtype,verbose=verbose)
            w[(lambda_,1.0)] = torch.from_numpy(np.nanmean(np.array(list(w_tmp.values())), axis=0))
