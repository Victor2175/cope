...
def leave_one_out_single(model_out,x,y,vars,\
                         ...
                         lr=1e-5,nb_gradient_iterations=50,verbose=True,dtype=torch.float32):
    ...
    # compute the weights
    if method == "robust":
        weights = compute_weights(training_models,w,x,y,notnan_idx,lambda_,mu_,dtype=dtype)
    else:
        weights = (1/len(training_models)) * torch.ones(len(training_models),dtype=dtype)
...
def leave_one_out_procedure(x,y,vars,\
                            ...
                            lr=1e-5,nb_gradient_iterations=20,verbose=True,dtype=torch.float32):
    ...
    for idx_m, m in enumerate(x.keys()):
        ...
        w[m], y_pred[m], y_test[m] = leave_one_out_single(m,x,y,vars,\
                                                            ...
                                                            lr,nb_gradient_iterations,verbose,dtype=dtype)
